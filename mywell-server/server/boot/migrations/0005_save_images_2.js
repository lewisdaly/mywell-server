...
const runMigration = (app) => {
  console.log("     - [0005_save_images] running migration 0005");
  let data = null; //base64 string
  let files = null; //list of file names

  return promiseReadDir('/usr/src/app/server/data/well_photos2/')
    .then(_data => {
      files = _data;
      ...
      return Promise.all(files.map(fileName => {
        ...
        return promiseReadFile(`/usr/src/app/server/data/well_photos2/${fileName}`)
          .then(_data => {
